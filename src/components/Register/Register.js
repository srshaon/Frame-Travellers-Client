...
const Register = () => {
    const { user, logOut, handleRegistration, handlePasswordChange, handleEmailChange, googleSignIn, setName, handleNameChange, error, setError } = useAuth();
    ...
    return (
        <div className="pt-5 registration-section ">
            <div className="h-100 pb-4 login-container d-flex flex-column justify-content-between">
                <div >
                    <div>
                        <h5 style={{ backgroundColor: '#c7ebe9' }} className="py-1">Register To Explore The Beauty Beyond Imagination</h5>
                    </div>
                    <div className="registration-div">
                        <form onSubmit={handleEmailRegistration} className="mb-1">
                            ...
                            <input className="login-submit-input" type="submit" value="Register" />


                        </form>
                        <span className=" or-span" >-----------Or-----------</span>
                        <br />
                        <button onClick={handleGoogleSignIn} className="google-sign-btn mt-1">Google Sign In</button>
                    </div>
                </div>
                <div style={{ backgroundColor: '#c7ebe9' }} className="mb-3 login-reg-div mt-3 py-1">
                    <span><strong style={{ fontSize: '18px' }}>Already Registered? <Link style={{ textDecoration: 'none', color: 'black' }} to='/login'> Click Here To Login </Link> </strong></span>
                </div>
            </div>
        </div>
    );
